...
SKETCHES ?= \
	examples/Stream/Stream.ino \
	examples/FileSystem/FileSystem.ino
TESTS ?= \
	test/test_progmemstream.cpp \
	test/test_progmemfilesystem.cpp
TEST_SOURCES ?= \
	src/ProgMemStream.cpp \
	src/ProgMemFileSystem.cpp
...
DEPLOY_ARDUINO_CMD ?= arduino-cli
...
define deploy-arduino
	$(DEPLOY_ARDUINO_CMD) \
		upload --verbose -b $(1) -p $(2) --input-file $(BUILD_DIR)/$(subst :,.,$(1))/$(basename $(3)).ino.hex
endef

define deploy-uf2
	$(DEPLOY_UF2_CMD) \
		"$(subst /,\,$(BUILD_DIR)/$(subst :,.,$(1)))" "$(DEPLOY_UF2_PORT)" $(2).ino.uf2
endef
...
deploy/attiny85:
	$(DEPLOY_ARDUINO_CMD) upload --verbose -b ATTinyCore:avr:attinyx5:chip=85 -P arduinoasisp --input-dir $(BUILD_DIR)/ATTinyCore.avr.attinyx5

.PHONY: deploy/attiny13
deploy/attiny13:
	$(DEPLOY_ARDUINO_CMD) upload --verbose -b MicroCore:avr:13 -P arduinoasisp --input-dir $(BUILD_DIR)/MicroCore.avr.13
